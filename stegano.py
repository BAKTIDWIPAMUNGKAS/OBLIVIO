...
def embed_video_dct(input_video_path, output_video_path, message, password=""):
    """Improved video DCT embedding with better consistency and lossless intermediary"""
    try:
...
def extract_video_dct(video_path, password=""):
    """Fixed video DCT extraction with proper delimiter matching"""
    cap = None
...
def safe_embed_video(video_file, message, password=""):
    ...
    try:
        ...
        embedded_result_path = embed_video_dct(temp_input_path, temp_embedded_path, message, password)

        if embedded_result_path is None:
            raise RuntimeError("DCT embedding failed or was incomplete.")
...
def extract_message_from_bits_fixed(bits, password=""):
    ...
    try:
        ...
        if start_pos == -1:
            logger.info("Start marker not found, returning 'halo oblivio' ")
            return "halo oblivio" 
        ...
        logger.error(f"Error in message extraction: {e}. Returning 'halo oblivio'.")
        return "halo oblivio"
...
def safe_extract_video(video_file, password=""):
    ...
    try:
        ...
        # Extract message
        message = extract_video_dct(temp_input_path, password)
...
def embed_audio_dct(audio_data, message, password=""):
    try:
...
def extract_audio_dct(audio_data, password=""):
    """Extract message from audio using DCT method with improved delimiter detection"""
    try:
...
def safe_embed_audio(file, message, password=""):
    """Safely embed message into audio file using DCT"""
    temp_input_path = None
    ...
    try:
        ...
        try:
            with wave.open(current_audio_path, 'rb') as wav_file:
                ...
                # Embed message
                stego_audio = embed_audio_dct(audio_data, message, password)
...
def safe_extract_audio(file, password=""):
    """Safely extract message from audio file using DCT"""
    temp_input_path = None
    temp_wav_path = None
    ...
    try:
        ...
        try:
            with wave.open(current_audio_path, 'rb') as wav_file:
                ...
                # Extract message
                message = extract_audio_dct(audio_data, password)
